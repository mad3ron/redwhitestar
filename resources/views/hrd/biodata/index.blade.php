        ...
        <table class="table table-report -mt-2">
            <thead>
                <tr>
                    ...
                    <th class="uppercase text-xs">Nama Jabatan</th>
                    <th class="uppercase text-xs">Nomor KTP/KK</th>
                    <th class="uppercase text-xs">Tempat Tgl Lahir</th>
                    <th class="uppercase text-xs">Agama</th>
                    <th class="uppercase text-xs">Status</th>
                    <th class="uppercase text-xs">Jenis</th>
                    <th class="uppercase text-xs">Alamat</th>
                    <th class="uppercase text-xs">RT/RW</th>
                    <th class="uppercase text-xs">Kelurahan - Kecamatan</th>
                    <th class="uppercase text-xs">Phone</th>
            ...
            <tbody>
                @foreach($biodatas as $biodata)
                    <tr class="intro-x">
                        ...
                        <td class="w-20">
                            <span class="uppercase text-xs whitespace-nowrap">{{ $biodata->nama }}</span>
                            <div class="uppercase text-xs whitespace-nowrap">{{ $biodata->jabatans->name }}</div>
                        </td>
                        <td class="w-20">
                            <span class="uppercase text-xs whitespace-nowrap">{{ $biodata->nik }}</span>
                            <div class="uppercase text-xs whitespace-nowrap">{{ $biodata->nokk }}</div>
                        </td>
                        <td class="w-20 uppercase text-xs whitespace-nowrap">
                            @if(isset($biodata->kotalahirs))
                                {{ $biodata->kotalahirs->tempat_lahir }}
                            @endif

                            <div class="uppercase text-xs whitespace-nowrap">{{ $biodata->tgl_lahir }}</div>
                        </td>
                        <td class="uppercase text-xs whitespace-nowrap">{{ $biodata->status }}</td>
                        <td class="uppercase text-xs whitespace-nowrap">{{ $biodata->agama }}</td>
                        <td class="uppercase text-xs whitespace-nowrap">{{ $biodata->jenis }}</td>
                        <td class="uppercase text-xs whitespace-nowrap">{{ $biodata->alamat }}</td>
                        <td class="w-20">
                            <span class="uppercase text-xs whitespace-nowrap">{{ $biodata->rt }}</span>
                            <div class="uppercase text-xs whitespace-nowrap">{{ $biodata->rw }}</div>
                        </td>
                        <td class="w-20 uppercase text-xs whitespace-nowrap">
                            @if(isset($biodata->kelurahans))
                                {{ $biodata->kelurahans->name}}
                                <div class="uppercase text-xs whitespace-nowrap">{{ $biodata->kelurahans->kecamatan }} - {{ $biodata->kelurahans->kabkota  }}</div>
                            @endif
                        </td>
                        <td class="uppercase text-xs whitespace-nowrap">{{ $biodata->phone }}</td>
                        <td class="w-20">
                            @if ($biodata->is_visible == 'Active')
                                <label class="flex items-left justify-left uppercase text-xs whitespace-nowrap text-success">Active</label>
                            @elseif ($biodata->is_visible =='Inactive')
                                <label class="flex items-left justify-left uppercase text-xs whitespace-nowrap text-primary">Inactive</label>
                            @elseif ($biodata->is_visible =='Disable')
                                <label class="flex items-left justify-left uppercase text-xs whitespace-nowrap text-warning">Disable</label>
                            {{-- @else
                        ...
                        <td class="table-report__action">
                            <div class="flex justify-center items-center">
                                <a href="{{ route('biodatas.edit', $biodata->id) }}" class="flex items-center text-primary uppercase text-xs mr-5" href="javascript:;"> <i data-lucide="edit" class="w-4 h-4 mr-1"></i> Edit </a>

                                    @if (Auth::user()->hasRole('super admin') || Auth::user()->hasRole('admin'))
                                        <form class="flex items-center uppercase text-xs text-danger"
                                            method="POST"
